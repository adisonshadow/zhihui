...
export function buildNovelAudiobookProjectPromptParts(
  ...
  const ephemeralSystemInstructions = [
    ...
    '· 对白：`type=dialogue`，须 speakerId + text + voice.tone（**风格指令**）；旁白叙述 **narration**；角色内心未出口台词 **innerVoice**（须画外音专用 characterId，见上）。',
    ...
    `· 【故事大纲音色】「大纲音色样本」表一行 ↔ novelScript 中一角色 + 界面固定旁白行。补齐/新增：**novel_script_list_characters**、**novel_script_upsert_character**（可选 novel_script_get_meta）；读大纲正文：**novel_get_episode**，episode_id="${NOVEL_OUTLINE_EPISODE_ID}"；绑定核对：**novel_audiobook_list_characters**。**勿把「旁白」写入 Script.characters。**`,
    '',
    '【MiMo V2.5 / 工作台合成细化】',
