...
export function buildNovelCoverPopoverProjectPrompt(opts?: {
  authorName?: string | null;
  /** 设置「每次生成封面图片的数量」，与工具校验一致 */
  coverCount?: number;
}): string {
  const author = normalizeNovelWriterAuthorName(opts?.authorName ?? undefined);
  const n = Math.min(Math.max(1, opts?.coverCount ?? 4), MAX_COVER_CANDIDATES);
  ...
  return [
    ...
    `每次 generate_images 须恰好 ${n} 条 prompts（与当前项目设置一致，至多 ${MAX_COVER_CANDIDATES}）；勿擅自改为 2 张或其它数量。`,
    '直接出方案，不要用户确认。',
    titleAndAuthorLine,
    '须使用下列工具，不要编造工具结果：',
    '1）novel_get_story_outline：开始时应先调用，获取故事大纲 Markdown。',
    `2）generate_images：在已获知书名前提下，写出恰好 ${n} 条互不重复、适合正方形（1:1）的小说封面 prompts，${generateImagesClause}；调用时必须同时传入 aspectRatio: "1:1" 与长度为 ${n} 的 prompts 数组。出图结果在对话界面展示。`,
