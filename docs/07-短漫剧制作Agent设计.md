...
- 利用本应用的**工具集**（剧情大纲、角色设计、视频设计器等）完成任务
...
| 能力 | key | 说明 | 依赖 |
|------|-----|------|------|
| **编剧** | script | 生成/修改剧本、概要、分镜、对白、旁白、动作 | 配置了 script 能力的模型 |
| **生成图** | draw | 生成角色形象、分镜背景、道具等图片 | 配置了 draw 能力的模型 |
| **抠图** | matting | 对图片执行智能抠图（火山引擎 `AIMattingConfig`、OpenAI 兼容 `images/edits` 模型、本地 ONNX） | 已配置抠图服务或具备 matting 的模型 |
...
| **生成执行脚本** | exec_script | 自动化执行脚本 | 配置了 exec_script 的模型 |
| **Agent 调度** | agent_orchestration | 主 Agent 优先选用的调度模型（见 `useAgentModel`） | 配置了该能力的模型；无则回退任意可调用模型 |
...
| **导出视频** | 将分镜合成为视频文件 | 提醒用户使用「导出视频」按钮，或说明导出参数 |
| **素材分类与管理** | 将素材归入角色/分镜/道具等 | 建议用户将某资源保存到某分类 |
...
| **结构诉求** | 「帮我写第 2 集，要有反转」 | 识别集/分镜、戏剧标签（reversal） |
| **风格诉求** | 「Q 版、古风、悬疑氛围」 | 画风、题材、基调 |
| **效率诉求** | 「快速出第一集分镜」「一键生成角色」 | 优先速度与可用性 |
| **修改诉求** | 「把小明这段台词改搞笑点」「分镜 2 加个钩子」 | 精确定位 path（见 短漫剧剧本元素说明 15.10） |
| **素材诉求** | 「生成主角形象」「这个图抠一下」 | 区分生成 vs 处理、单图 vs 序列 |

### 3.2 诉求解析与澄清

- **模糊诉求**：主动追问关键信息（如题材、时长、风格），避免盲目生成
- **复杂诉求**：拆解为多个子任务，按「编剧 → 角色 → 分镜 → 素材」顺序执行
...
- **当前项目**：项目名、横竖屏、已有集/分镜、角色列表
- **当前焦点**：用户在哪个 Tab（剧情大纲 / 角色设计 / 视频设计器）、选中的集/分镜
- **已有内容**：当前集的概要/剧本、当前分镜的素材条、角色形象等
...
| 特点 | 说明 | Agent 应用 |
|------|------|------------|
| **快速** | 单集时长短（1–数分钟），用户期待快速看完、快速制作 | 剧本不宜过长，分镜数量适中，单分镜时长可控 |
| **高效讲故事** | 信息密度高，少废话，直奔主题 | 对白精简，旁白画龙点睛，动作说明简洁 |
| **节奏快** | 快切、强节奏，避免拖沓 | 分镜转换干脆，镜头建议偏短 |
| **爽点多** | 反转、冲突、高潮、钩子要足 | 主动为分镜打 dramaTags，建议每集末尾有钩子 |
...
- 分类：角色、分镜背景、情景道具、声效、透明视频特效、音乐、贴纸（功能文档 5.1）
...
| 工具名 | 说明 | 参数示例 | 返回值 |
|--------|------|----------|--------|
| `get_project_context` | 获取当前项目上下文 | projectId | 项目信息、集列表、角色列表、当前选中集/分镜 |
...
| `get_characters` | 获取项目角色列表 | projectId | 角色 id、名称、形象路径等 |
| `generate_character_image` | 调用绘图模型生成角色形象 | projectId, characterId?, prompt, style? | 图片路径/error |
| `list_assets` | 列出项目素材 | projectId, category? | 素材列表 |
| `matting_image` | 对图片执行抠图 | imagePath, configId? | 抠图后图片路径 |
| `get_scene_timeline` | 获取分镜时间线信息 | projectId, episodeIndex, sceneIndex | 素材条、分层、关键帧等 |
...
你的任务是帮助用户在剧情大纲、角色设计、视频设计器等模块中高效完成创作。
```
...
剧本结构：集 → 分镜 → 对白/旁白/动作。支持戏剧标签（悬念、钩子、冲突、反转、高潮等）。
```
...
| 角色设计 + AI 绘画 | Agent 可调用绘图工具生成形象并绑定角色 |
