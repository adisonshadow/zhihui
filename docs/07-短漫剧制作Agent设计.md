...
| **生成图** | draw | 生成角色形象、场景背景、道具等图片 | 配置了 draw 能力的模型 |
...
| **素材分类与管理** | 将素材归入角色/场景/道具等 | 建议用户将某资源保存到某分类 |
...
- **当前项目**：项目名、横竖屏、已有集/场景、角色列表
- **当前焦点**：用户在哪个 Tab（剧情大纲 / 角色设计 / 视频设计器）、选中的集/场景
- **已有内容**：当前集的概要/剧本、当前场景的素材条、角色形象等
...
- 分类：角色、场景背景、情景道具、声效、透明视频特效、音乐、贴纸（功能文档 5.1）
...
| 工具名 | 说明 | 参数示例 | 返回值 |
|--------|------|----------|--------|
| `get_project_context` | 获取当前项目上下文 | projectId | 项目信息、集列表、角色列表、当前选中集/场景 |
